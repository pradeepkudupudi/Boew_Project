    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-background font-mono text-sm relative overflow-hidden">
      ...
      <div className="w-full max-w-md p-8 border border-border bg-card relative z-10 shadow-2xl">
        <div className="flex items-center gap-3 mb-8 pb-4 border-b border-border">
          ...
          <h1 className="text-xl font-bold tracking-widest text-primary">BOEW_SYS // LOGIN</h1>
        </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="pt-4 flex flex-col gap-4">
              <Button type="submit" disabled={loginMutation.isPending} className="w-full rounded-none font-bold tracking-widest uppercase group relative overflow-hidden">
                <span className="relative z-10">{loginMutation.isPending ? "AUTHENTICATING..." : "INITIATE_SESSION"}</span>
              ...
              <div className="text-center text-xs text-muted-foreground">
                UNREGISTERED OPERATIVE? <Link href="/register" className="text-primary hover:underline">REQUEST_ACCESS</Link>
              </div>
            </div>
          </form>
        </Form>
